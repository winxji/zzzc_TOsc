void eb_plot_CLs_GaussCLs()
{
  ...
  roostr_CLs = "outfile_CLs_nueDIS_95.root";
  roostr_GaussCLs = "za_roofile_GaussCLs_nueDIS.root";

  roostr_CLs = "outfile_CLs_nueAPP_95.root";
  roostr_GaussCLs = "za_roofile_GaussCLs_nueAPP.root";
  ...
  roostr = "h2d_results";
  TH2D *h2d_results = new TH2D(roostr, "", 10, 1e-4, 1, 10, 0.1, 100);
  h2d_results->Draw();
  ...
  gh_sensitivity_median->Draw("same l"); 

  gh_gauss_pred->Draw("same l"); gh_gauss_pred->SetLineColor(kRed); gh_gauss_pred->SetLineWidth(3);
  ...
  gh_gauss_data->Draw("same l"); gh_gauss_data->SetLineColor(kMagenta); gh_gauss_data->SetLineWidth(3);
  ...
  lg->AddEntry( gh_gauss_pred, "Gaussian CLs + Asimov: sensitivity", "l");
